...
결과 화면
  │
  ├─ [오늘 카드 확정하기 / 최종 카드 확인하기] → 최종 카드 화면
  │
     ...
     - 재뽑기는 하루 1회만 가능해요.
        ...
        └─ [광고 보고 뽑기]
           ...
           광고 로딩 (개발 환경: 1.5초 딜레이 시뮬레이션)
              │
              ├─ 광고 성공 → 뽑기 화면으로 이동 (상자 애니메이션 자동 시작)
              │               1.2초 후 새 카드 공개 → 결과 화면
              │               (결과 화면에서 "최종 카드 확인하기" 버튼으로 최종 화면 이동)
              │
...
- 광고 성공 직후 뽑기 화면(`DrawScreen`)으로 전환되며 `isRedraw=true` props 전달
- `isRedraw=true`이면:
  - 헤더: "아직 더 운이 남았어요!"
  - 설명: "새로운 카드를 뽑는 중이에요"
  - 하단 안내 박스 숨김 (일반 뽑기 안내 불필요)
  - 상자 애니메이션 자동 시작 (`isDrawing=true` 상태로 진입)
- 1.2초 애니메이션 후 새 카드와 함께 결과 화면으로 전환
